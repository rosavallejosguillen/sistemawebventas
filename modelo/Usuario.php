...
class Usuario
{
	...
	/*$sql="SELECT * FROM usuario inner join tipo_us on us_tipo=id_tipo_us and id_usuario";*/
		$query = $this->acceso->prepare($sql);
	...
	function buscar(){
		if (!empty($_POST['consulta'])) {
			...
			$sql="SELECT * FROM usuario inner join tipo_us on us_tipo=id_tipo_us where nombre_us NOT LIKE'' ORDER BY id_usuario LIMIT 25";
			$query=$this->acceso->prepare($sql);
			$query->execute();
			$this->objetos=$query->fetchall();
			return $this->objetos;
