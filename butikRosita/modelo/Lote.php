	...
	function buscar(){
		if (!empty($_POST['consulta'])) {
			...
			$sql="SELECT id_lote,stock,vencimiento,concentracion,adicional,producto.nombre as prod_nom,laboratorio.nombre as lab_nom,tipo_producto.nombre as tip_nom,presentacion.nombre as pre_nom,proveedor.nombre as proveedor,producto.avatar as logo FROM lote 
				inner join proveedor on lote_id_prov=id_proveedor 
				inner join producto on lote_id_prod=id_producto 
				inner join laboratorio on prod_lab=id_laboratorio 
				inner join tipo_producto on prod_tip_prod=id_tip_prod 
				inner join presentacion on prod_present=id_presentacion and producto.nombre like :consulta order by producto.nombre limit 25";
			$query=$this->acceso->prepare($sql);
			...
			$sql="SELECT id_lote,stock,vencimiento,concentracion,adicional,producto.nombre as prod_nom,laboratorio.nombre as lab_nom,tipo_producto.nombre as tip_nom,presentacion.nombre as pre_nom,proveedor.nombre as proveedor,producto.avatar as logo FROM lote 
				...
				inner join laboratorio on prod_lab=id_laboratorio 
				inner join tipo_producto on prod_tip_prod=id_tip_prod 
				inner join presentacion on prod_present=id_presentacion and producto.nombre not like '' order by producto.nombre limit 25";
			$query=$this->acceso->prepare($sql);
			$query->execute();
			$this->objetos=$query->fetchall();
			return $this->objetos;
